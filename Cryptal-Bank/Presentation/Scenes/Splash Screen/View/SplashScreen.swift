...
//  Cryptal-Bank
//
//  Created by Irakli Chkhitunidzde on 14.02.22.
//
...
struct DigitalRain: View {
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 0.1)) { timeline in
            DigitalRainCanvas(date: timeline.date).background(Color.black)
        }
    }
...
class MatrixCore: ObservableObject {
    ...
    static var randomOrigin: UnitPoint {
        UnitPoint(x: .random(in: -0.25...1.25), y: .random(in: -1...1.25))
    }
...
struct runApp: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIViewController {
        ...
        if UDManager.isNotFirstInstall(){
            ...
            vc = sb.instantiateViewController(withIdentifier: "AppIntroNavigationController")
            vc.modalPresentationStyle = .fullScreen
        }else{
            
            let sb = UIStoryboard(name: "LogInViewController", bundle: nil)
            vc = sb.instantiateViewController(withIdentifier: "LogInNavigationController")
            vc.modalPresentationStyle = .fullScreen
        }
        return vc
    }
